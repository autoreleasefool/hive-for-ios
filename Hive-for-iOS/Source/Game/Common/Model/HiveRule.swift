...
enum HiveRule: CaseIterable {
	...
	var description: String {
		...
		case .movement: return
			...
			rule. A player cannot move their pieces until they have [placed](rule:placement) their [queen](class:Queen).
			If a player is ever unable to move or place a piece, they must [pass their turn](rule:passing).
			If they have any moves available, then they **must** move. The [pill bug](class:Pill Bug)
			adds additional complexity to moving pieces and should be explored separately.
			"""
...
extension Piece.Class {
	var rules: String {
		...
		case .beetle: return
			...
			also move **on top of the hive**, immobilizing the piece beneath it and forming a [stack](rule:stacks).
			A [stack](rule:stacks) becomes the color of whichever piece is on top. Beetles must respect the
			[freedom of movement](rule:freedomOfMovement) rule while on top of the hive, and beetles are capable of
			**stacking multiple pieces high**. They can return to the ground at any time,
			so long as they are not blocked.
			"""
		case .hopper: return
			"""
		...
		case .ladyBug: return
			...
			the final move returning the piece to the ground. The lady bug must **always finish on the ground**.
			While moving onto or down from the hive, the lady bug must respect the
			[freedom of movement](rule:freedomOfMovement) rule.
			"""
		case .mosquito: return
			...
			opponent's [ant](class:Ant) and a friendly [queen](class:Queen) can move as either an [ant](class:Ant)
			or a [queen](class:Queen) on that turn. Mosquitoes can copy a beetle and move on top of the hive.
			If it does, it **remains a beetle until it returns to the ground**. A mosquito which is adjacent only
			to another mosquito **has no moves to copy** and therefore cannot be moved. When adjacent to a
			[stack](rule:stacks) a mosquito can only copy the moves of the piece on top.
			"""
		case .pillBug: return
			"""
			Pill Bugs can only move one space at a time, similar to the [queen](class:Queen). However, they also
			have a **special ability** which allows them to take any piece which is adjacent to it, friendly or not,
			move it up on top of the pill bug's back, and then back down to an adjacent, free space.
			The pill bug **cannot move pieces which are stacked**. The piece being moved must respect the
			[freedom of movement](rule:freedomOfMovement) as it moves up and back down. Finally, a piece which has
			just moved or been moved by the pill bug **cannot be moved on the next turn**, nor can a pill bug
			move a piece which has just moved or been moved in the previous turn.
			"""
		case .queen: return
			"""
			The queen can only move one space at a time. It must respect the
			[freedom of movement](rule:freedomOfMovement) rule when it moves. The game is over when one player is
			able to surround the opponent's queen on all 6 sides with pieces from **either player**. A player
			cannot move any other piee until their queen has been [placed](rule:placement).
			"""
		case .spider: return
			"""
